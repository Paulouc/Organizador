...
RUTA = Path(
    input(
        "Ingresa la ruta completa de tu carpeta (ejemplo: C:/Users/TuUsuario/Downloads): "
    )
)
...
def calcular_hash_sha256(ruta_archivo):
    ...
    with ruta_archivo.open("rb") as f:
        ...
        ):  # lee el archivo en bloques de 4096 bytes
...
for item in RUTA.iterdir():
    ...
    if item.is_file():
        try:
            ...
            if hash_archivo in archivos_vistos:
                ...
                if fecha_modificacion > fecha_original:
                    ...
                    print(
                        f"Duplicado eliminado: {item.name}. Se conserva la versión original: {nombre_original}"
                    )
                    item.unlink()
        ...
        if extension in CATEGORIAS:
            ...
            if destino.exists():
                # Verificar si es duplicado exacto (hash igual)
                hash_destino = calcular_hash_sha256(destino)
                if hash_destino == hash_archivo:
                    print(f"Duplicado detectado, eliminando: {item.name}")
                    item.unlink()
                else:
                    # Si no es duplicado, renombrar con sufijo de fecha/hora
                    nuevo_nombre = f"{destino.stem}_{fecha_modificacion.strftime('%Y%m%d%H%M%S')}{destino.suffix}"
                    destino = carpeta_destino / nuevo_nombre
                    item.rename(destino)
                    print(
                        f"Archivo renombrado y movido: {item.name} --> {destino.name}"
                    )
            else:
                item.rename(destino)
